...
export async function generateFullReportExcel(
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    row.values = [
      ...
      !isTgdd && it['Doanh thu tiền mặt'] !== null ? Math.round(Number(it['Doanh thu tiền mặt'])) : '',
      !isTgdd && it['Doanh thu tiền mặt trước thuế'] !== null ? Math.round(Number(it['Doanh thu tiền mặt trước thuế'])) : '',
      isTgdd ? Math.round(Number(it['Công nợ'] || 0)) : '',
